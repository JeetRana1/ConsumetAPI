...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  fastify.get('/:query', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const res = await jaGet(
        `${JUSTANIME_BASE}/search/suggestions?query=${encodeURIComponent(query)}`,
        {
          headers: {
            'User-Agent': UA,
  ...
  fastify.get('/info', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const res = redis
        ? await cache.fetch(redis as Redis, `justanime:info:${id}`, fetchInfo, REDIS_TTL)
        : await fetchInfo();
