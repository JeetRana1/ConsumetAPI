...
class SatoruProvider extends AnimeParser {
  ...
  private readonly preferWindowsCurl =
    ...
    !['1', 'true', 'yes'].includes(
      String(process.env.SATORU_DISABLE_CURL || '').toLowerCase(),
    );
  ...
  async fetchAnimeInfo(id: string): Promise<IAnimeInfo> {
    ...
    $('a').each((_, el) => {
      ...
      const slugPart =
        absoluteUrl.split('/watch/')[1]?.split('?')[0]?.replace(/\/$/, '') || '';
      if (!slugPart) return;
      ...
      const sharedTokenCount = candidateTitleTokens.filter((token) =>
    ...
    if (movieId) {
      ...
      const episodeDataStr = await this.fetch(
        `${this.baseUrl}/ajax/episode/list/${movieId}`,
        {
      ...
      try {
        ...
        $eps('.ep-item').each((i, el) => {
          ...
          animeInfo.episodes?.push({
            ...
            title:
              ep.find('.ep-name').text().trim() || `Episode ${ep.attr('data-number')}`,
            url: epUrl,
          });
        });
      } catch {
  ...
  async fetchEpisodeServers(episodeId: string): Promise<IEpisodeServer[]> {
    ...
    const dataStr = await this.fetch(
      `${this.baseUrl}/ajax/episode/servers?episodeId=${normalizedEpisodeId}`,
      {
        'X-Requested-With': 'XMLHttpRequest',
        Referer: this.baseUrl,
      },
  ...
  async fetchEpisodeSources(episodeId: string, serverId?: string): Promise<ISource> {
    ...
    const tryServer = async (
      candidate: string,
    ): Promise<{ data: any; resolvedServerId: string }> => {
      const dataStr = await this.fetch(
        `${this.baseUrl}/ajax/episode/sources?id=${candidate}`,
        {
          'X-Requested-With': 'XMLHttpRequest',
          Referer: this.baseUrl,
        },
...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  ...
  const getSatoruSlug = (episodeId: string) =>
  ...
  const toEpisodeNum = (ep: any): number => {
    const n = Number(
      ep?.number ?? ep?.episode ?? ep?.episodeNumber ?? ep?.episodeNum ?? 0,
    );
    return Number.isFinite(n) ? n : 0;
  };
  const sanitizeDirectNoDash = (payload: any): ISource | null => {
    if (!payload || typeof payload !== 'object') return null;
    const direct = (Array.isArray(payload?.sources) ? payload.sources : []).filter(
      (src: any) => {
  ...
  const scoreSatoruCandidate = (
    ...
    const searchTitle = String(
      candidateSearchRow?.title || candidateSearchRow?.name || candidateInfo?.title || '',
    );
    ...
    const normalizedCandidateId = normalizeTitle(
      String(candidateInfo?.id || '')
        .split(':')[0]
        .replace(/-/g, ' '),
    );
    if (normalizedHiSlug && normalizedCandidateId) {
      if (normalizedHiSlug === normalizedCandidateId) score += 140;
        ...
        normalizedCandidateId.includes(normalizedHiSlug) ||
  ...
  const scoreRelatedSeasonEntry = (
    ...
    for (const hiTitle of hiVariants) {
      ...
      if (normalizedHi === normalizedEntry) score += 120;
        ...
        normalizedHi.includes(normalizedEntry) ||
  ...
  const getSeasonAliases = (title: string, season: number): string[] => {
    ...
    if (
      normalizedTitle.includes('demon slayer') ||
  ...
  const scoreResolvedSatoruEntry = (
    ...
    const yearValue = Number(
      String(info?.season || '').match(/\b(19|20)\d{2}\b/)?.[0] || 0,
    );
  ...
  const resolveSatoruByMetadata = async ({
    ...
    const knownSettled = await Promise.allSettled(
      knownCandidateIds.map(
        async (candidateId) => await satoru.fetchAnimeInfo(candidateId),
      ),
    );
      ...
      for (const candidate of orderedKnown) {
        ...
        if (remainingKnownEpisode <= candidate.len) {
          ...
          const exactKnownEpisode =
            ...
            knownEpisodes[
              Math.max(0, Math.min(knownEpisodes.length - 1, remainingKnownEpisode - 1))
            ];
    ...
    const searchSettled = await Promise.allSettled(
      searchTerms.map((term) => satoru.search(term, 1)),
    );
    ...
    for (const info of baseInfos) {
      ...
      const relatedSettled = await Promise.allSettled(
        relatedRows
          .slice(0, 8)
          .map(async (row: any) => await satoru.fetchAnimeInfo(String(row.id))),
      );
    ...
    const sameSeasonCandidates = ranked
      ...
      .filter(
        (info) =>
          detectSeasonNumber(info?.id, info?.title, info?.japaneseTitle) === season,
      );
  ...
  const resolveSatoruSeriesByTitle = async ({
    ...
    const searchSettled = await Promise.allSettled(
      searchTerms.slice(0, 6).map((term) => satoru.search(term, 1)),
    );
    for (const settled of searchSettled) {
      if (settled.status !== 'fulfilled') continue;
      const rows = Array.isArray(settled.value?.results) ? settled.value.results : [];
    ...
    const familyRoot = (value: string) =>
      normalizeLooseTitle(value).split(' ').slice(0, 4).join(' ');
    ...
    const ordered = [...allInfos.values()]
      .filter((info) => {
        ...
        return (
          joined.includes(titleNorm.split(' ').slice(0, 2).join(' ')) ||
      ...
      .map((info) => ({
        ...
        episodes: (Array.isArray(info?.episodes) ? info.episodes : []).map(
          (ep: any, idx: number) => ({
            id: String(ep?.id || ''),
            number: idx + 1,
