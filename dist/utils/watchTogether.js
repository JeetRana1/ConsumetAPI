...
exports.registerWatchTogether = void 0;
...
function broadcast(code, type, payload = {}, exceptId) {
...
function broadcastPublicRooms() {
    ...
    clients.forEach((client) => sendToClient(client, 'rooms:public', { rooms: roomsPayload }));
}
...
function leaveClient(client, options = {}) {
    if (!client?.roomCode) {
...
function handleClientMessage(client, msg) {
    ...
    if (msg.type === 'room:create') {
        ...
        client.name = String(msg.name || client.name || `Host-${client.id.slice(0, 4).toUpperCase()}`).slice(0, 32);
                ...
                if (other.id === previousUserId && other.id !== client.id) {
                    ...
                    if (other.kind === 'ws' && other.ws) {
                        try {
                            ...
                            console.log('[wp] ws.close error', e);
                        }
                    }
                    if (other.ws && wsToClient.has(other.ws))
                        wsToClient.delete(other.ws);
    ...
    if (msg.type === 'host:promote') {
        const room = client.roomCode ? rooms.get(client.roomCode) : null;
        if (!room || room.hostId !== client.id || room.hostAway)
            return;
    ...
    if (msg.type === 'host:state') {
        ...
        broadcast(room.code, 'player:sync', { action, ...getRoomStateSnapshot(room) }, client.id);
        return;
    }
    if (msg.type === 'host:control') {
        ...
        if (!room || room.hostId !== client.id)
            return;
        broadcast(room.code, 'control:sync', {
    ...
    if (msg.type === 'change_media') {
        ...
        broadcast(room.code, 'media_changed', {
...
function getOrCreateHttpClient(clientId) {
...
function registerWatchTogether(fastify) {
    ...
    fastify.get('/watch-party/poll', async (request) => {
    ...
    fastify.post('/watch-party/action', async (request, reply) => {
        ...
        const client = getOrCreateHttpClient(clientId);
        if (body.name)
            client.name = String(body.name).slice(0, 32);
        const msg = { ...body };
        delete msg.clientId;
        delete msg.name;
    ...
    wss.on('connection', (socket) => {
        ...
        socket.on('close', () => {
            ...
            leaveClient(client);
            if (wsToClient.get(socket) === client.id)
                wsToClient.delete(socket);
...
exports.registerWatchTogether = registerWatchTogether;
