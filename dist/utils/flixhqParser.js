...
exports.parseMixedSection = parseMixedSection;
function parseHome($) {
    const slider = [];
...
function parsePaginatedResults($, selector) {
    ...
    const lastPage = Number(paginationElement
...
exports.parsePaginatedResults = parsePaginatedResults;
...
function parseInfo($) {
    ...
    const mediaInfo = {
        ...
        genre: $('.row-line:has(span:contains("Genre:")) a')
            ...
            .get()
            .map((v) => v.trim()) || null,
        casts: $('.row-line:has(span:contains("Casts:")) a')
            .map((i, el) => $(el).text().trim())
            .get() || null,
        duration: $('div.stats span.item.mr-3').eq(2).text().trim() || null,
        score: Number($('div.stats span.item.mr-3').eq(1).text().trim()) || null,
        country: $('.row-line:has(span:contains("Country:")) a')
            .map((i, el) => $(el).text().trim())
            .get() || null,
        production: $('.row-line:has(span:contains("Production:"))')
            .text()
            .replace('Production:', '')
            .replace(/\s+/g, ' ')
            .split(',')
...
exports.parseSeasons = parseSeasons;
...
exports.parseEpisodes = parseEpisodes;
