...
var flixhqParser_exports = {};
__export(flixhqParser_exports, {
  parseEpisodes: () => parseEpisodes,
  parseHome: () => parseHome,
  parseInfo: () => parseInfo,
  parseItems: () => parseItems,
  parseMixedSection: () => parseMixedSection,
  parsePaginatedResults: () => parsePaginatedResults,
  parseSearchSuggestions: () => parseSearchSuggestions,
  parseSeasons: () => parseSeasons,
  parseServers: () => parseServers
});
module.exports = __toCommonJS(flixhqParser_exports);
...
function parseMixedSection($, selector) {
  const items = [];
  $(selector).each((_, element) => {
    const type = $(element).find("span.float-right.fdi-type").text().trim();
    const baseData = {
...
function parsePaginatedResults($, selector) {
  ...
  $(selector).each((_, element) => {
    ...
    const baseData = {
      ...
      name: $(element).find("h2.film-name, h3.film-name").first().text().replace(/\s+/g, " ").trim() || $(element).find("img.film-poster-img").attr("alt")?.replace(/\s+Watch Online.*$/i, "").trim() || null,
      posterImage: $(element).find("img.film-poster-img.lazyload").attr("data-src") || $(element).find("img.film-poster-img").attr("src") || null,
      quality: $(element).find("div.pick.film-poster-quality").text().trim() || null,
      type
    };
    if (type === "Movie") {
...
function parseInfo($) {
  ...
  $("div.block_area-content.block_area-list.film_list.film_list-grid div.flw-item").each(
    (_, element) => {
      const type2 = $(element).find("span.float-right.fdi-type").text().trim();
      const baseData = {
        id: extractIdFromHref(
          $(element).find("a.film-poster-ahref.flw-item-tip").attr("href")
        ),
  ...
  const pageHref = headingLink.attr("href") || $('link[rel="canonical"]').attr("href") || $('meta[property="og:url"]').attr("content");
  const id = extractIdFromHref(pageHref) || null;
  const type = id?.includes("tv") || id?.includes("series") ? "TV" : "Movie";
  const structuredTitle = $('script[type="application/ld+json"]').map((_, el) => {
    try {
      const json = JSON.parse($(el).contents().text());
      const graphItems = Array.isArray(json?.["@graph"]) ? json["@graph"] : [json];
      const mediaItem = graphItems.find(
        (item) => ["Movie", "TVSeries", "Series"].includes(String(item?.["@type"] || ""))
      );
  ...
  const mediaInfo = {
    id,
    name: headingLink.text().replace(/\s+/g, " ").trim() || structuredTitle || $('meta[property="og:title"]').attr("content")?.replace(/^Watch\s+/i, "").replace(/\s+FlixHQ.*$/i, "").trim() || $("img#image_storage").attr("alt")?.trim() || null,
    posterImage: $("img#image_storage").attr("data-img") || $("img#image_storage").attr("src") || $("div.w_b-cover").attr("style")?.match(/url\(["']?(.*?)["']?\)/)?.[1] || null,
    type,
...
function parseSeasons($) {
  const legacy = $(`.dropdown-menu > a`).map((_, el) => {
...
function parseEpisodes($, seasonNumber, mediaId) {
  const legacy = $(`.nav > li`).map((_, el) => {
  ...
  if (legacy.length)
    return legacy;
  return $(`.eps-item`).map((_, el) => {
    const anchor = $(el).is("a") ? $(el) : $(el).find("a");
    const rawId = anchor.attr("data-id") || $(el).attr("data-id") || anchor.attr("id")?.split("-")[1] || extractIdFromHref(anchor.attr("href"));
    if (!rawId)
      return null;
    const rawNumber = anchor.attr("data-number") || $(el).attr("data-number") || anchor.attr("href")?.match(/s(\d+)-e(\d+)/i)?.[2] || anchor.text() || anchor.attr("title") || "";
    ...
    return {
      episodeId: String(rawId).startsWith("episode-") ? rawId : `${mediaId.replace("watch-", "")}-episode-${rawId}`,
      title: episodeTitle,
      episodeNumber,
