...
const routes = async (fastify, options) => {
    ...
    const normalizeTitleForMatch = (v) => String(v || '')
        .normalize('NFKD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/\(([^)]*)\)/g, ' ')
    ...
    const parseStatusFromEpisode = (ep) => {
        ...
        const boolCanon = ep?.isMangaCanon === true ||
            ep?.isCanon === true ||
            ep?.mangaCanon === true ||
            ep?.canon === true;
        const text = normalizeTitleForMatch(`${ep?.title || ''} ${ep?.description || ''} ${ep?.type || ''} ${ep?.category || ''}`);
    ...
    fastify.get('/proxy', async (request, reply) => {
        ...
        try {
            ...
            if (/(?:(?:shiora|mikora)\.(?:top|site|club|net)|lostproject\.club)$/i.test(target.hostname) &&
            ...
            const fetchWithChain = async (targetUrl, requestConfig) => {
                ...
                if (effectiveChain.length <= 1) {
                    try {
                        const proxyOptions = (0, outboundProxy_1.toAxiosProxyOptions)(effectiveChain[0]);
                        const upstream = await axios_1.default.get(targetUrl, {
                            proxy: false,
                            ...requestConfig,
                            ...proxyOptions,
                        });
                ...
                const attemptProxy = async (proxyUrl) => {
                    ...
                    const isAnimesaltCdn = /(^|\.)(as-cdn\d+|z\d+|animesalt|as2|as-api)\.(pro|ac|top|xyz|link|click|net|cc|org)$/i.test(target.hostname);
                    const isHianimeCdn = /(^|\.)(rainveil\d*|megacloud\d*|rapid-cloud\d*|rabbitstream\d*|vizcloud\d*|cloud9|bunnycdn|vidcloud)\.(xyz|tv|ru|net|gg|co|online|pro|ac|cc|bz|li|to)$/i.test(target.hostname);
                    if (isAnimesaltCdn) {
    ...
    fastify.get('/vtt', async (request, reply) => {
        ...
        try {
            const { getCachedSubtitleText } = await Promise.resolve().then(() => __importStar(require('./browserRuntimeExtractor.js')));
            ...
            for (const targetRequest of targetRequests) {
                for (const candidateReferer of refererCandidates) {
                    ...
                    for (const proxyUrl of chain) {
                        try {
                            const { toAxiosProxyOptions: tap } = await Promise.resolve().then(() => __importStar(require('./outboundProxy.js')));
