...
const resolveHdstream4uTvEpisodeId = async (request, id, type, season, episode) => {
    ...
    try {
        ...
        const searchResults = await Promise.all(titleCandidates.map((title) => searchHdhub4uByTitle(`${title} ${targetSeasonLabel}`).catch(() => [])));
    ...
    const isBonusEntry = (entry) => String(entry?.category || '').toLowerCase() === 'bonus' ||
    ...
    const match = entries.find((entry) => !isBonusEntry(entry) &&
    ...
    const episodeOnlyMatches = numberedEntries.filter((entry) => Number(entry?.episodeNumber || entry?.episode || entry?.number || 0) === requestedEpisode);
    if (episodeOnlyMatches.length === 1) {
        return normalizeEpisodeId(episodeOnlyMatches[0]);
    }
    const seasonValues = Array.from(new Set(numberedEntries
...
const loadAnimeIdIndex = async () => {
    ...
    animeIdIndexLoading = (async () => {
        try {
            const { data } = await axios_1.default.get(ANIME_MAPPING_URL, {
                timeout: 30000,
                responseType: 'json',
            });
...
const resolveHdstream4uEpisodeId = async (request, mediaInfo) => {
    ...
    const searchResults = await Promise.all(titleCandidates.map((title) => searchHdhub4uByTitle(title).catch(() => [])));
    ...
    if (!matchedUrl) {
        const sitemapUrl = await findBestHdhub4uUrl(titleCandidates, Number.isFinite(preferredYear) ? preferredYear : undefined);
    ...
    if (!isTv) {
        ...
        if (directFileId)
            return directFileId;
        const watchOnline = servers.find((server) => /hubstream\.(?:art|pw|cc|ink|foo|boo)\/#/i.test(String(server?.url || '')));
        if (watchOnline?.url)
...
const buildAnimesaltTmdbInfo = async (request, id, type) => {
    const baseTmdb = new extensions_1.META.TMDB(main_1.tmdbApi);
...
const routes = async (fastify, options) => {
    ...
    fastify.get('/:query', async (request, reply) => {
        ...
        const tmdb = configureMeta(new extensions_1.META.TMDB(main_1.tmdbApi));
        try {
    ...
    const getDirectTmdbSearch = async (query, page = 1) => {
        try {
            ...
            const [multiRes, movieRes, tvRes] = await Promise.all(['multi', 'movie', 'tv'].map((kind) => axios_1.default.get(`https://api.themoviedb.org/3/search/${kind}?api_key=${main_1.tmdbApi}&query=${encodedQuery}&page=${page}`, { headers })));
            ...
            for (const [kind, response] of [
                ...
                for (const item of Array.isArray(response.data?.results) ? response.data.results : []) {
                    if (item?.id === undefined || item?.id === null || item.media_type === 'person')
                        continue;
            ...
            if (merged.size) {
                ...
                return {
                    ...
                    total_results: Math.max(Number(multiRes.data?.total_results || 0), Number(movieRes.data?.total_results || 0), Number(tvRes.data?.total_results || 0)),
                    total_pages: Math.max(Number(multiRes.data?.total_pages || 0), Number(movieRes.data?.total_pages || 0), Number(tvRes.data?.total_pages || 0)),
                };
            }
        }
        catch (err) {
    ...
    const watch = async (request, reply) => {
            ...
            try {
                ...
                if (infoRes.statusCode < 400) {
                    ...
                    if (candidates.length) {
                        ...
                        const raced = await Promise.any(candidates.map(async (candidateId) => {
                            const payload = await withSoftTimeout(hdstream4uProvider_1.HdStream4uProvider.fetchSources(candidateId, 'hdstream4u', false, { mediaId: String(id) }), 30000);
                        ...
                        if (raced) {
                            if (cacheKey && main_1.redis) {
                                main_1.redis
                                    .setex(cacheKey, main_1.REDIS_TTL, JSON.stringify(raced))
                                    .catch(() => { });
                            }
        ...
        if ((type === 'movie' || type === 'tv') &&
            ...
            id) {
            try {
                const discoveryTmdb = new extensions_1.META.TMDB(main_1.tmdbApi, undefined);
                ...
                if (titleCandidates.length) {
                    try {
                        const hdstreamEpisodeId = type === 'tv'
                            ? episodeId
