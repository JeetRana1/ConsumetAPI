...
const hdhub4u_1 = __importDefault(require("../../providers/movies/hdhub4u"));
const provider = new hdhub4u_1.default();
const routes = async (fastify, _options) => {
    fastify.get('/', async (_request, reply) => {
        reply.status(200).send({
            intro: 'Welcome to the custom HDHub4U provider',
    ...
    fastify.get('/:query', async (request, reply) => {
        ...
        try {
            const res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `hdhub4u:search:${query}:${page}`, async () => await provider.search(query, page), main_1.REDIS_TTL)
    ...
    fastify.get('/info', async (request, reply) => {
        ...
        try {
            const res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `hdhub4u:info:${id}`, async () => await provider.fetchMediaInfo(id), main_1.REDIS_TTL)
