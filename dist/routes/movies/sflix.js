...
const routes = async (fastify, options) => {
    const sflix = (0, provider_1.configureProvider)(new extensions_1.MOVIES.SFlix());
    fastify.get('/', (_, rp) => {
        rp.status(200).send({
    ...
    fastify.get('/:query', async (request, reply) => {
        ...
        let res = main_1.redis
            ? await cache_1.default.fetch(main_1.redis, `sflix:${query}:${page}`, async () => await sflix.search(query, page ? page : 1), main_1.REDIS_TTL)
            : await sflix.search(query, page ? page : 1);
        reply.status(200).send(res);
    });
    fastify.get('/recent-shows', async (request, reply) => {
        let res = main_1.redis
            ? await cache_1.default.fetch(main_1.redis, `sflix:recent-shows`, async () => await sflix.fetchRecentTvShows(), main_1.REDIS_TTL)
            : await sflix.fetchRecentTvShows();
        reply.status(200).send(res);
    });
    fastify.get('/recent-movies', async (request, reply) => {
        let res = main_1.redis
            ? await cache_1.default.fetch(main_1.redis, `sflix:recent-movies`, async () => await sflix.fetchRecentMovies(), main_1.REDIS_TTL)
    ...
    fastify.get('/trending', async (request, reply) => {
        ...
        try {
            ...
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `sflix:trending:${type}`, async () => type === 'tv'
                    ? await sflix.fetchTrendingTvShows()
                    : await sflix.fetchTrendingMovies(), main_1.REDIS_TTL)
                : type === 'tv'
                    ? await sflix.fetchTrendingTvShows()
                    : await sflix.fetchTrendingMovies();
            reply.status(200).send(res);
    ...
    fastify.get('/info', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `sflix:info:${id}`, async () => await sflix.fetchMediaInfo(id), main_1.REDIS_TTL)
    ...
    fastify.get('/watch', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `sflix:watch:${episodeId}:${mediaId}:${server}`, async () => await (0, streamable_1.fetchWithServerFallback)(async (selectedServer) => await sflix.fetchEpisodeSources(episodeId, mediaId, selectedServer), server, streamable_1.MOVIE_SERVER_FALLBACKS), main_1.REDIS_TTL)
                : await (0, streamable_1.fetchWithServerFallback)(async (selectedServer) => await sflix.fetchEpisodeSources(episodeId, mediaId, selectedServer), server, streamable_1.MOVIE_SERVER_FALLBACKS);
            const promoted = await (0, embedToDirect_1.promoteEmbedSourcesToDirect)(sflix, res, server);
            ...
            try {
                const fallback = await (0, movieServerFallback_1.getMovieEmbedFallbackSource)(sflix, episodeId, mediaId, server);
                if (fallback) {
                    const promotedFallback = await (0, embedToDirect_1.promoteEmbedSourcesToDirect)(sflix, fallback, server);
    ...
    fastify.get('/servers', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `sflix:servers:${episodeId}:${mediaId}`, async () => await sflix.fetchEpisodeServers(episodeId, mediaId), main_1.REDIS_TTL)
    ...
    fastify.get('/country/:country', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `sflix:country:${country}:${page}`, async () => await sflix.fetchByCountry(country, page), main_1.REDIS_TTL)
    ...
    fastify.get('/genre/:genre', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `sflix:genre:${genre}:${page}`, async () => await sflix.fetchByGenre(genre, page), main_1.REDIS_TTL)
