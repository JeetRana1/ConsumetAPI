...
const PROD_DIRECT_RACE_TIMEOUT_MS = Number(process.env.SATORU_PROD_DIRECT_RACE_TIMEOUT_MS || '') || 9000;
class SatoruProvider extends models_1.AnimeParser {
    constructor() {
        super(...arguments);
        this.name = 'Satoru';
        this.baseUrl = 'https://satoru.one';
    ...
    async fetch(url, headers = {}) {
        ...
        if (this.preferWindowsCurl) {
            try {
                const curlArgs = ['-sS', '-L', '--compressed', '-A', userAgent];
    ...
    async fetchAnimeInfo(id) {
        ...
        $('a').each((_, el) => {
            ...
            const sharedPrefix = currentTitleTokens.length >= 2 &&
            ...
            const titleFamilyMatch = currentTitleFamily.includes(candidateTitleFamily) ||
        ...
        animeInfo.related = [...relatedMap.values()];
        if (movieId) {
            ...
            try {
                ...
                const $eps = (0, cheerio_1.load)(episodeData.html || '');
        ...
        try {
            ...
            console.error('Failed to get AniList ID for', animeInfo.title, e.message);
        }
        return animeInfo;
    }
    ...
    async fetchEpisodeSources(episodeId, serverId) {
        const normalizedEpisodeId = this.normalizeEpisodeId(episodeId);
        ...
        try {
            ...
            for (const srv of servers) {
                ...
                if (id && !candidateServerIds.includes(id))
                    candidateServerIds.push(id);
            }
        }
        catch {
            // If server list endpoint fails, we'll still try any provided serverId.
        }
        if (!candidateServerIds.length)
        ...
        const tryServer = async (candidate) => {
            ...
            if (!link)
                throw new Error(`Server ${candidate} returned no link`);
        ...
        if (data?.outro)
            result.outro = data.outro;
        if (data?.skip)
            result.skip = data.skip;
        if (data?.skips)
            result.skips = data.skips;
        if (data?.timestamps)
...
const routes = async (fastify, options) => {
    const satoru = (0, provider_1.configureProvider)(new SatoruProvider());
    const localEpisodeMapCache = new Map();
    const EPISODE_MAP_TTL_MS = 60 * 60 * 1000;
    const isSatoruBlockedError = (err) => {
    ...
    const normalizeAnimeIdForFallback = (id) => String(id || '').split(':')[0];
    const getSatoruSlug = (episodeId) => String(episodeId || '').split('$episode$')[0];
    const normalizeEpisodeIdForWatch = (id) => {
    ...
    const firstSuccessful = async (tasks) => {
        return await new Promise((resolve, reject) => {
            ...
            let lastError = new Error('All strategies failed');
    ...
    const detectSeasonNumber = (...values) => {
        for (const rawValue of values) {
            ...
            if (/\bfinal season\b/i.test(value))
                return 99;
    ...
    const detectSeasonPart = (...values) => {
        for (const rawValue of values) {
            const value = normalizeTitle(String(rawValue || ''));
            if (!value)
                continue;
            const numericMatch = value.match(/\bpart\s+(\d{1,2})\b/i) ||
    ...
    const collectTitleVariants = (payload, fallback) => {
    ...
    const countSharedWords = (left, right) => {
    ...
    const scoreSatoruCandidate = (candidateInfo, candidateSearchRow, hiInfo, hiSlug, hiEpisodeCount) => {
        ...
        for (const hiTitle of hiVariants) {
            for (const candidateTitle of candidateVariants) {
                ...
                if (!normalizedHiTitle || !normalizedCandidateTitle)
                    continue;
                if (normalizedHiTitle === normalizedCandidateTitle)
                    score += 120;
                else if (normalizedHiTitle.includes(normalizedCandidateTitle) ||
        ...
        if (hiSeason !== null && candidateSeason !== null) {
            if (hiSeason === candidateSeason)
                score += 90;
            else
                score -= Math.min(120, Math.abs(hiSeason - candidateSeason) * 60);
        }
    ...
    const scoreRelatedSeasonEntry = (entry, hiInfo, hiSlug, hiEpisodeCount) => {
        const hiVariants = collectTitleVariants(hiInfo, slugToTitle(hiSlug));
        ...
        if (hiSeason !== null && entrySeason !== null) {
            if (hiSeason === entrySeason)
                score += 220;
            else
                score -= Math.min(220, Math.abs(hiSeason - entrySeason) * 90);
        }
        if (hiPart !== null && entryPart !== null) {
            if (hiPart === entryPart)
                score += 140;
            else
                score -= Math.min(180, Math.abs(hiPart - entryPart) * 90);
        }
    ...
    const normalizeLooseTitle = (value) => String(value || '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
    ...
    const getSeasonAliases = (title, season) => {
        ...
        if (normalizedTitle.includes('demon slayer') || normalizedTitle.includes('kimetsu no yaiba')) {
            ...
            if (season === 3)
                return ['swordsmith village arc'];
            if (season === 4)
                return ['hashira training arc'];
        }
        return [];
    };
    ...
    const getSeasonSubEntryOrder = (title, season, info) => {
        ...
        if (normalizedTitle.includes('demon slayer') || normalizedTitle.includes('kimetsu no yaiba')) {
            if (season === 2) {
                if (haystack.includes('mugen train'))
                    return 0;
                if (haystack.includes('entertainment district'))
                    return 1;
            }
        }
    ...
    const scoreResolvedSatoruEntry = (info, { title, season, seasonLabel, preferredYear, }) => {
        ...
        if (season > 1) {
            if (candidateSeason === season)
                score += 320;
            else if (candidateSeason !== null)
                score -= Math.abs(candidateSeason - season) * 180;
            else
                score -= 80;
        }
        ...
        if (targetPart !== null && candidatePart !== null) {
            if (candidatePart === targetPart)
                ...
                score -= Math.abs(candidatePart - targetPart) * 180;
        }
        if (targetSeasonLabel) {
            ...
            if (candidateJoined.includes(targetSeasonLabel))
                score += 260;
        ...
        if (preferredYear && yearValue) {
            if (preferredYear === yearValue)
                score += 40;
            else if (Math.abs(preferredYear - yearValue) === 1)
                score += 15;
        }
        return score;
    };
    const candidateMatchesRequestedSeason = (info, { title, season, seasonLabel, }) => {
        if (season <= 1)
            return true;
        const variants = collectTitleVariants(info, info?.title);
        const candidateSeason = detectSeasonNumber(info?.id, ...variants);
        const aliases = getSeasonAliases(title, season);
        const candidateHaystack = normalizeLooseTitle([info?.id, ...variants, info?.japaneseTitle].filter(Boolean).join(' '));
        const normalizedSeasonLabel = normalizeSeasonLabel(seasonLabel);
        if (candidateSeason === season)
            return true;
        if (normalizedSeasonLabel && candidateHaystack.includes(normalizedSeasonLabel))
            return true;
        if (aliases.some((alias) => candidateHaystack.includes(alias)))
            return true;
        return false;
    };
    const resolveSatoruByMetadata = async ({ title, season, episode, seasonLabel, preferredYear, }) => {
        ...
        const candidateIds = new Set(knownCandidateIds);
        const knownSettled = await Promise.allSettled(knownCandidateIds.map(async (candidateId) => await satoru.fetchAnimeInfo(candidateId)));
        ...
        if (!candidateIds.size)
            throw new Error('No Satoru candidates found');
        const fetched = await Promise.allSettled([...candidateIds].map(async (candidateId) => await satoru.fetchAnimeInfo(candidateId)));
        const baseInfos = fetched
            .filter((entry) => entry.status === 'fulfilled')
            .map((entry) => entry.value)
            .filter((info) => Array.isArray(info?.episodes) && info.episodes.length);
        if (!baseInfos.length)
            throw new Error('No Satoru info candidates found');
        ...
        const strictCandidates = [...allInfos.values()].filter((info) => candidateMatchesRequestedSeason(info, {
            title,
            season,
        ...
        if (!picked)
            throw new Error('Failed to resolve exact Satoru season');
        ...
        if (sameSeasonCandidates.length > 1) {
            ...
            const pickedLabelHaystack = normalizeSeasonLabel([picked?.id, picked?.title, picked?.japaneseTitle].filter(Boolean).join(' '));
            const hasExplicitPickedLabel = normalizedRequestedSeasonLabel &&
        ...
        const targetEpisode = episodes.find((ep) => toEpisodeNum(ep) === episode) ||
            episodes[Math.max(0, Math.min(episodes.length - 1, episode - 1))];
        if (!targetEpisode?.id)
            throw new Error('Failed to resolve Satoru episode');
        return {
            anime: {
                id: picked.id,
                title: picked.title,
    ...
    const resolveSatoruSeriesByTitle = async ({ title, preferredYear, }) => {
        ...
        if (!candidateIds.size)
            throw new Error('No Satoru series candidates found');
        const fetched = await Promise.allSettled([...candidateIds].map(async (candidateId) => await satoru.fetchAnimeInfo(candidateId)));
        const baseInfos = fetched
            .filter((entry) => entry.status === 'fulfilled')
            .map((entry) => entry.value)
            .filter((info) => Array.isArray(info?.episodes) && info.episodes.length);
        if (!baseInfos.length)
            throw new Error('No Satoru series infos found');
        const titleNorm = normalizeLooseTitle(title);
        const familyRoot = (value) => normalizeLooseTitle(value).split(' ').slice(0, 4).join(' ');
        const selectedBase = baseInfos
            ...
            .sort((left, right) => right.score - left.score)[0]?.info;
        if (!selectedBase)
            throw new Error('Failed to choose Satoru series base');
        const allInfos = new Map();
        ...
        const relatedSettled = await Promise.allSettled(relatedRows.map(async (row) => await satoru.fetchAnimeInfo(String(row.id))));
        for (const related of relatedSettled) {
            if (related.status !== 'fulfilled')
                continue;
        ...
        const ordered = [...allInfos.values()]
            ...
            episodes: (Array.isArray(info?.episodes) ? info.episodes : []).map((ep, idx) => ({
                id: String(ep?.id || ''),
                number: idx + 1,
    ...
    fastify.get('/', (_, rp) => {
        rp.status(200).send({
            intro: "Welcome to the Satoru provider: check out the provider's website @ https://satoru.one/",
    ...
    fastify.get('/:query', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `satoru:search:${query}:${page}`, async () => await satoru.search(query, page), main_1.REDIS_TTL)
    ...
    fastify.get('/info/:id', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `satoru:info:${id}`, async () => await satoru.fetchAnimeInfo(id), main_1.REDIS_TTL)
    ...
    fastify.get('/resolve', async (request, reply) => {
        ...
        if (!Number.isFinite(season) || season <= 0)
            return reply.status(400).send({ message: 'season is invalid' });
        if (!Number.isFinite(episode) || episode <= 0)
            return reply.status(400).send({ message: 'episode is invalid' });
        try {
            const cacheKey = `satoru:resolve:${title}:${season}:${episode}:${seasonTitle}:${year}`;
            const resolved = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, cacheKey, async () => await resolveSatoruByMetadata({
                    title,
                    season,
                    episode,
    ...
    fastify.get('/series', async (request, reply) => {
        ...
        try {
            ...
            const payload = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, cacheKey, async () => await resolveSatoruSeriesByTitle({
                    title,
                    preferredYear: Number.isFinite(year) && year > 1900 ? year : undefined,
                }), main_1.REDIS_TTL)
    ...
    fastify.get('/watch/:episodeId', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `satoru:watch:${normalizedEpisodeId}:${serverId}`, async () => await satoru.fetchEpisodeSources(normalizedEpisodeId, serverId), main_1.REDIS_TTL)
    ...
    fastify.get('/servers/:episodeId', async (request, reply) => {
        ...
        try {
            let res = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `satoru:servers:${episodeId}`, async () => await satoru.fetchEpisodeServers(episodeId), main_1.REDIS_TTL)
