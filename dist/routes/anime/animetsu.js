...
exports.getStreamLinks = exports.getEpisodes = exports.search = void 0;
...
const animetsuGet = async (path, config = {}) => {
    const url = path.startsWith('http') ? path : `${API_BASE}${path}`;
    const referer = String(config.headers?.Referer || `${BASE_URL}/`);
    return (0, outboundProxy_1.proxyGet)(url, {
        timeout: 8000,
        ...config,
        headers: {
...
const pickTitle = (title, fallback = '') => {
...
const normalizeSeasonRows = (rows) => {
    ...
    return rows
        ...
        return {
            ...
            image: normalizeImage(item),
            cover: normalizeCover(item),
            season: Number(item?.season || item?.seasonNumber || item?.number || index + 1) ||
                index + 1,
            releaseDate: item?.seasonYear || item?.year || item?.start_date,
            anilistId: item?.anilist_id || item?.anilistId,
            malId: item?.mal_id || item?.malId,
...
const normalizeProviderRelatedRows = (rows) => {
    if (!Array.isArray(rows))
        return [];
    return rows
        .map((item) => {
...
const makeEpisodeId = (animeId, episode) => `${animeId}$episode$${episode}`;
...
const normalizeEpisodeRows = (animeId, payload) => {
    ...
    return episodesRaw
        .map((ep, index) => {
        const number = Number(ep?.ep_num || ep?.number || ep?.episode || ep?.ep || index + 1) ||
            index + 1;
        return {
        ...
        .filter((ep) => ep.id && ep.episode)
        .sort((a, b) => Number(a.episode) - Number(b.episode));
};
const scrapeEpisodesFromHtml = (animeId, html) => {
    ...
    $('[href*="/watch/"], [data-ep], [data-episode], [data-id], [data-episode-id]').each((_, el) => {
        ...
        const dataEp = node.attr('data-ep') ||
        ...
        if (!number || found.has(number))
            return;
        const providerEpisodeId = extractHashId(node.attr('data-id') ||
        ...
        found.set(number, {
            ...
            title: node.attr('title') ||
                ...
                `Episode ${number}`,
            image: absoluteUrl(node.find('img').first().attr('src') ||
...
const shouldProbeSource = (source) => {
    const url = String(source?.url || '').trim();
    if (!url || source?.isM3U8 !== true)
        return false;
    if (/\.m3u8(?:$|\?)/i.test(url))
        return false;
    return /swiftstream\.top\/proxy\/oppai\//i.test(url);
};
const probePlayableSource = async (source) => {
    if (!shouldProbeSource(source))
        return source;
    try {
        const response = await (0, outboundProxy_1.proxyGet)(source.url, {
        ...
        if (response.status < 200 || response.status >= 300)
            return null;
        if (!isLikelyHlsManifest(String(response.data || ''), contentType))
            return null;
        return source;
    }
    catch {
...
const getEpisodes = async (animeId) => {
    ...
    if (!id || !HASH_RE.test(id))
        throw new Error('Animetsu anime id is invalid');
    try {
    ...
    const page = await (0, outboundProxy_1.proxyGet)(`${BASE_URL}/anime/${encodeURIComponent(id)}`, {
        headers: animetsuHeaders(`${BASE_URL}/anime/${id}`),
        timeout: 8000,
    });
    const episodes = scrapeEpisodesFromHtml(id, String(page.data || ''));
    if (!episodes.length)
        throw new Error(`No Animetsu episodes found for ${id}`);
    return episodes;
};
exports.getEpisodes = getEpisodes;
const getStreamLinks = async (episodeId, options = {}) => {
    ...
    const jobs = servers.flatMap((server) => sourceTypes.map(async (sourceType) => {
        const startedAt = Date.now();
        try {
            const res = await animetsuGet(`/oppai/${encodeURIComponent(animeId)}/${encodeURIComponent(episode)}?server=${encodeURIComponent(server)}&source_type=${encodeURIComponent(sourceType)}`, {
            ...
            let sources = (Array.isArray(payload.sources) ? payload.sources : [])
                ...
                return {
                    ...
                    isM3U8: source?.type === 'video/mpegurl' ||
                        source?.old_hls === true ||
                        /\.m3u8(?:$|\?)/i.test(url),
                    referer: `${BASE_URL}/`,
                    headers: {
...
const getInfo = async (id) => {
    ...
    const [infoRes, episodes] = await Promise.all([
        ...
        (0, exports.getEpisodes)(animeId).catch(() => []),
    ]);
    const info = infoRes.data || {};
    const { seasons: relatedSeasons, relations: relatedRelations, recommendations, characters, staff, studios, ...baseInfo } = info;
...
const routes = async (fastify, options) => {
    ...
    const searchHandler = async (request, reply) => {
        ...
        try {
            const fetchSearch = () => (0, exports.search)(query);
            const data = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `animetsu:net:search:${query}`, fetchSearch, main_1.REDIS_TTL)
                : await fetchSearch();
    ...
    fastify.get('/info', async (request, reply) => {
        ...
        try {
            ...
            const data = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, `animetsu:net:info:v1:${extractHashId(id)}`, fetchInfo, main_1.REDIS_TTL)
                : await fetchInfo();
            reply.status(200).send(data);
        }
        catch (err) {
    ...
    fastify.get('/servers/:id/:episode', async (request, reply) => {
        const { id, episode } = request.params;
        const animeId = extractHashId(id);
        try {
    ...
    fastify.get('/watch/:episodeId', async (request, reply) => {
        ...
        try {
            ...
            const fetchWatch = () => (0, exports.getStreamLinks)(episodeId, { server: query.server, sourceType });
            const { animeId, episode } = parseEpisodeId(episodeId);
            const cacheKey = `animetsu:net:watch:v1:${animeId}:${episode}:${query.server || 'auto'}:${sourceType || 'both'}`;
            const data = main_1.redis
                ? await cache_1.default.fetch(main_1.redis, cacheKey, fetchWatch, main_1.REDIS_TTL)
                : await fetchWatch();
            reply.status(200).send(data);
        }
        catch (err) {
