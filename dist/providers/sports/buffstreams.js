...
class BuffStreams extends models_1.MovieParser {
    get baseUrl() { return domain_resolver_1.BaseUrlResolver.getBaseUrl(); }
    constructor() {
        super();
        ...
        this.DEEP_PROBE_TTL_MS = 5 * 60 * 1000;
        this.deepProbeCache = new Map();
        this.probeInFlight = new Map();
        this.logo = `${this.baseUrl}/images/mlb.webp?v3e32`;
    }
    get homeUrl() { return `${this.baseUrl}/index7`; }
    get index18Url() { return `${this.baseUrl}/index18`; }
    getAllCategoryPages(b) {
        return {
            nfl: `${b}/nflstreams2`,
    ...
    extractLiveState(title = '', statusText = '', rowHtml = '') {
        try {
            const normalize = (value) => this.cleanText(value)
    ...
    buildEasternEventStartMs(exactTime, canonicalDate) {
        ...
        if (isAmTime && resultMs < Date.now() - 8 * 60 * 60 * 1000) {
            ...
            if (nextDayMs > Date.now() - 2 * 60 * 60 * 1000)
                return nextDayMs;
        }
        return resultMs;
    }
    ...
    inferLiveState(title, statusText, sectionTitle) {
        ...
        return /\bin progress\b|\blive\b|\b1st half\b|\b2nd half\b|\bhalftime\b|\bquarter\b|\bq[1-4]\b|\bperiod\b|\bovertime\b|\binnings?\b|\btop \d+|\bbottom \d+|\bpractice\b|\bqualifying\b|\bsprint\b|\brace\b/i.test(haystack);
    }
    parseCompetitionItem(itemHtml, fallbackImage, sectionTitle) {
        ...
        const nameMatches = [
            ...anchorHtml.matchAll(/<span[^>]*class=["'][^"']*name[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi),
        ]
    ...
    parseStreamsFromHTML(html) {
        ...
        for (let i = 0; i < starts.length; i += 1) {
            ...
            const titleMatch = headingBlock.match(/<h2[^>]*class=["'][^"']*league-name[^"']*["'][^>]*>([\s\S]*?)<\/h2>/i) || headingBlock.match(/<h[1-4][^>]*>([\s\S]*?)<\/h[1-4]>/i);
    ...
    async deepProbeStream(streamUrl) {
        ...
        const promise = (async () => {
            try {
                ...
                const countdownMatch = result.eventStartUtcMs > 0
                    ? null
                ...
                const hasM3u8 = /src\s*[:=]\s*["']?[^"'\s]*\.m3u8/i.test(html) ||
    ...
    async search(query, options) {
        try {
            ...
            if (!raw || raw === 'all') {
                ...
                const results = await Promise.allSettled(urls.map((url) => this.fetchCategoryStreams(url)));
                streams = this.mergeStreams(results
                    .filter((r) => r.status === 'fulfilled')
                    .map((r) => r.value));
                if (categoryKey === 'fighting') {
                    streams = streams.filter((s) => /\/(?:boxing|mma|ufc|pfl|wwe)\b|title-game\/(?:boxing|mma|ufc|pfl|wwe)|\b(?:boxing|mma|ufc|pfl|wwe|bkfc|bellator|one fight|fighting championship)\b/i.test(`${s.url || ''} ${s.title || ''} ${s.sectionTitle || ''}`));
                }
                else if (raw && raw.startsWith('category:')) {
                    streams = streams.filter((s) => String(s.type || '').toLowerCase() === categoryKey ||
                        `${s.url || ''} ${s.title || ''} ${s.sectionTitle || ''}`
                            .toLowerCase()
                            .includes(categoryKey));
                }
                else if (raw) {
                    streams = streams.filter((s) => `${s.title} ${s.statusText || ''} ${s.type} ${s.sectionTitle || ''}`
                        .toLowerCase()
                        .includes(raw));
                }
            }
    ...
    async fetchMediaInfo(mediaId) {
        ...
        try {
            ...
            const title = titleMatch?.[1]?.replace(/\s*-\s*Buffstreams\s*$/i, '').trim() ||
                'BuffStreams Event';
            ...
            const countdownMatch = eventStartUtcMs > 0
                ? null
                : countdownFromScripts
                    ? [
                        '',
            ...
            const hasM3u8InPage = /src\s*[:=]\s*["']?[^"'\s]*\.m3u8/i.test(html) ||
                /(?:file|source|manifest|streamUrl)\s*[:=]\s*["']?[^"'\s]*\.m3u8/i.test(html) ||
                /https?:\/\/[^"'\s]+\.m3u8(?:\?[^"'\s]*)?/i.test(html);
            const pageTextCheck = html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
            const hasInProgressCheck = /\bIN\s*PROGRESS\b|\b2ND\s*QUARTER\b|\b1ST\s*QUARTER\b|\b3RD\s*QUARTER\b|\b4TH\s*QUARTER\b|\b1ST\s*HALF\b|\b2ND\s*HALF\b|\bHALFTIME\b|\bOVERTIME\b/i.test(pageTextCheck);
    ...
    async fetchEpisodeSources(episodeId) {
        ...
        try {
            ...
            if (!gatheredSources.length && embedUrl) {
                const playwrightSources = await (0, browserRuntimeExtractor_1.extractDirectSourcesWithPlaywright)(embedUrl, pageUrl, 12000).catch(() => []);
    ...
    collectEmbedCandidates(html, pageUrl) {
        ...
        for (const match of source.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
            ...
            if (/server|source|play|stream|watch|embed/i.test(href) ||
...
exports.BuffStreams = BuffStreams;
exports.default = BuffStreams;
